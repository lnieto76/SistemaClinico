...
if($_GET['id1'])
{
	if(isset($_POST['Aceptar_color_letra']))
	{
	if($si=mysql_query("update configuracion set color_letra='$_GET[id1]'"))
	{
		echo "<script>alert('Color de LETRA del sistema modificado correctamente');window.close()</script>";
	}
	else
	{
		echo mysql_error();
...
<input type=submit name='Aceptar_color_letra' value='SI'><a href=javascript:window.close(); alt=cerrar ventana>No</a>
</form>
";

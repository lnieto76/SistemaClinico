...
<form method=post>
<table align=center>
<tr><td colspan=2>Busqueda de pacientes registrados entr 2 fechas</td></tr>
<tr><td>Digite la fecha 1</td><td><input type=text name=fecha1></td></tr>
<tr><td>Digite la fecha 2</td><td><input type=text name=fecha2></td></tr>
...
if($_POST)
{
	$fechauno = explode ('/',$_POST['fecha1']);
			...
			$ano = $fechauno[2];
			$sep="-";
			$fecha1 = $ano.$sep.$mes.$sep.$dia;
			
	$fechados = explode ('/',$_POST['fecha2']);
			$dia2= $fechados[0];
			$mes2 = $fechados[1];
			$ano2 = $fechados[2];
			$fecha2 = $ano2.$sep.$mes2.$sep.$dia2;
